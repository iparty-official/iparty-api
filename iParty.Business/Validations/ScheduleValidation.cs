...
namespace iParty.Business.Validations
{
    public class ScheduleValidation : AbstractValidator<Schedule>, IScheduleValidation
    {
        public ScheduleValidation()
        {
            ...
            RuleForEach(x => x.Hours).Must(hour => hour.InitialHour >=0  && hour.InitialHour <= 24).WithMessage("A hora inicial precisa estar entre 0 e 24.");

            RuleForEach(x => x.Hours).Must(hour => hour.FinalHour >=0 && hour.FinalHour <= 24).WithMessage("A hora final precisa estar entre 0 e 24.");
        }        
    }
}
